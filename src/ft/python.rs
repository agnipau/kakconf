...
pub const VALUES: [&str; 5] = ["True", "False", "None", "self", "inf"];

pub const META: [&str; 2] = ["import", "from"];
...
pub const KEYWORDS: [&str; 32] = [
    ...
    "else", "except", "exec", "finally", "for", "global", "if", "in", "is", "lambda", "nonlocal",
...
pub fn python() -> anyhow::Result<String> {
...
declare-option str-list python_static_words {keywords_all}

# Highlight keywords.
add-highlighter shared/python/code/ regex '\b({values})\b' 0:value
add-highlighter shared/python/code/ regex '\b({meta})\b' 0:meta
add-highlighter shared/python/code/ regex '\b({attributes})\b' 0:attribute
...
"#,
        keywords_all = format!(
            "{values} {meta} {attributes} {methods} {exceptions} {keywords} {types} {functions}",
            values = VALUES.join(" "),
            meta = META.join(" "),
