...
const FILETYPES: [&str; 3] = ["c", "cpp", "objc"];

const C_KEYWORDS: [&str; 17] = [
    "asm", "break", "case", "continue", "default", "do", "else", "for", "goto", "if", "return",
    "sizeof", "switch", "while", "offsetof", "alignas", "alignof",
];

const C_ATTRIBUTES: [&str; 14] = [
    "auto",
    "atomic",
    "const",
    "enum",
...
const CPP_VALUES: [&str; 5] = ["NULL", "false", "nullptr", "this", "true"];

const C_CPP_BUILTIN_MACROS: [&str; 7] = [
    "__cplusplus",
    "__STDC_HOSTED__",
    "__FILE__",
    "__LINE__",
...
const OBJC_VALUES: [&str; 9] = [
    "FALSE", "NO", "NULL", "TRUE", "YES", "id", "nil", "self", "super",
];

const OBJC_DECORATORS: [&str; 12] = [
    "autoreleasepool",
    "catch",
    "class",
    "end",
...
declare-option str-list c_static_words {c_all}

# Highlight keywords.
add-highlighter shared/c/code/keywords regex %[\b({c_keywords})\b] 0:keyword
add-highlighter shared/c/code/attributes regex %[\b({c_attributes})\b] 0:attribute
add-highlighter shared/c/code/types regex %[\b({c_types})\b] 0:type
add-highlighter shared/c/code/values regex %[\b({c_macros})\b] 0:value
...
# Add the language's grammar to the static completion list.
declare-option str-list cpp_static_words {cpp_all}

# Highlight keywords.
add-highlighter shared/cpp/code/keywords regex %[\b({cpp_keywords})\b] 0:keyword
add-highlighter shared/cpp/code/attributes regex %[\b({cpp_attributes})\b] 0:attribute
add-highlighter shared/cpp/code/types regex %[\b({cpp_types})\b] 0:type
add-highlighter shared/cpp/code/values regex %[\b({cpp_values})\b] 0:value

# C and C++ compiler macros.
add-highlighter shared/c/code/macros regex %[\b({c_cpp_builtin_macros})\b] 0:builtin
add-highlighter shared/cpp/code/macros regex %[\b({c_cpp_builtin_macros})\b] 0:builtin

# Objective-C specific.
add-highlighter shared/objc/code/number regex %[\b-?\d+[fdiu]?|'((\\.)?|[^'\\])'] 0:value

# Add the language's grammar to the static completion list.
declare-option str-list objc_static_words {objc_all}

# Highlight keywords.
add-highlighter shared/objc/code/keywords regex %[\b({objc_keywords})\b] 0:keyword
add-highlighter shared/objc/code/attributes regex %[\b({objc_attributes})\b] 0:attribute
add-highlighter shared/objc/code/types regex %[\b({objc_types})\b] 0:type
add-highlighter shared/objc/code/values regex %[\b({objc_values})\b] 0:value
add-highlighter shared/objc/code/decorators regex %[@({objc_decorators})\b] 0:attribute
...
define-command -hidden c-family-insert-include-guards %[
    evaluate-commands %sh[
        kakconf c-family-insert-include-guards -g "$kak_opt_c_include_guard_style"
    ]
]
...
define-command -hidden c-family-alternative-file %[
    evaluate-commands %sh[
        kakconf c-family-alternative-file
    ]
]
...
"#,
        ...
        c_keywords = C_KEYWORDS.join("|"),
        c_attributes = C_ATTRIBUTES.join("|"),
        c_types = C_TYPES.join("|"),
        c_macros = C_MACROS.join("|"),
        cpp_all = format!(
            "{} {} {} {} {}",
            CPP_KEYWORDS.join(" "),
            CPP_ATTRIBUTES.join(" "),
        ...
        cpp_keywords = CPP_KEYWORDS.join("|"),
        cpp_attributes = format!("{} {}", CPP_ATTRIBUTES.join("|"), CPP_ENTITIES.join("|")),
        cpp_types = CPP_TYPES.join("|"),
        cpp_values = CPP_VALUES.join("|"),
        c_cpp_builtin_macros = C_CPP_BUILTIN_MACROS.join("|"),
        objc_all = format!(
            "{} {} {} {} {}",
            OBJC_KEYWORDS.join(" "),
            OBJC_ATTRIBUTES.join(" "),
            OBJC_TYPES.join(" "),
            OBJC_VALUES.join(" "),
            OBJC_DECORATORS.join(" "),
        ),
        objc_keywords = OBJC_KEYWORDS.join("|"),
        objc_attributes = OBJC_ATTRIBUTES.join("|"),
        objc_types = OBJC_TYPES.join("|"),
        objc_values = OBJC_VALUES.join("|"),
        objc_decorators = OBJC_DECORATORS.join("|"),
    )?;
